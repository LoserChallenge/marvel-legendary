...
function initCityArrays() {
  ...
  // Heroes of Asgard — reset per-game scheme state (Moral Failings / Guardians Defeated counters +
  // the Eternal Darkness city/HQ flag arrays). Guarded so it no-ops if the HoA file is absent.
  if (typeof hoaResetSchemeState === "function") hoaResetSchemeState();
}
  ...
  selectedVillains.forEach((villainName) => {
    ...
    if (villain) {
      villain.cards.forEach((card) => {
        ...
        // Preserve original type for Locations and Heroes of Asgard Villainous Weapons
        // (a Weapon revealed from the Villain Deck must route to hoaCaptureWeaponOnPlay,
        // not enter the city as a plain Villain); default to "Villain" for regular cards
        const cardType =
          modifiedCard.type === "Location"
            ? "Location"
            : modifiedCard.type === "Villainous Weapon"
              ? "Villainous Weapon"
              : "Villain";
...
async function initGame(heroes, villains, henchmen, mastermindName, scheme) {
  ...
  // Heroes of Asgard — same rationale as the SWV1 block above. The HoA per-turn flags
  // (royalDecreeDebuffActive, usherToValhallaArmed, helasCloakUsedThisTurn, flyingStallionEndTurnDraw,
  // jormungandHQDiscountedHeroes) are cleared by hoaResetTurnFlags() at endTurn ONLY, so a game that
  // ends mid-turn (victory / Evil Wins) would leak them into a new game started without a page reload.
  // hoaResetGameFlags() clears all five — it is NOT the same as hoaResetTurnFlags(), which deliberately
  // leaves flyingStallionEndTurnDraw alone (that runs before the end-of-turn draw that consumes it).
  if (typeof hoaResetGameFlags === "function") hoaResetGameFlags();
  ...
  // B25 — clear every Mastermind's Heroes of Asgard capturedWeapons per game. The `masterminds` array
  // holds module-level DB objects that persist across an in-app new game (no page reload), and
  // hoaMastermindCapturedWeapons() pushes Weapons onto the BASE object (never the transient
  // {...base,...base.epic} overlay, which getSelectedMastermind() re-derives every render). A game
  // ending with the Mastermind holding a Weapon would otherwise leak it into the next game — phantom
  // Attack bonus (hoaCapturedWeaponAttack @ recalculateMastermindAttack) and a phantom Artifact handed
  // out on defeat. Clear ALL masterminds, not just the selected one, so switching Masterminds between
  // games can't strand stale Weapons on the one that's picked two games later. Mutate in place
  // (.length = 0) so any overlay still holding the old array reference sees the clear.
  // Iterate the array directly rather than looking the base up by name: `epic` overrides `name`
  // ("Epic Hela, Goddess of Death"), so an overlay-name lookup would miss the base object entirely.
  // (B16/B17/B21 cross-game-leak pattern.)
  if (typeof masterminds !== "undefined" && Array.isArray(masterminds)) {
    masterminds.forEach((m) => {
      if (m && Array.isArray(m.capturedWeapons)) m.capturedWeapons.length = 0;
    });
  }
  ...
  if (villainCard.ambushEffect && villainCard.ambushEffect !== "None") {
    ...
    if (typeof ambushEffectFunction === "function") {
      ...
      if (negate) {
        ...
        // Heroes of Asgard — Valkyrie's Flying Stallion: when an Ambush is played, BEFORE it takes
        // effect, the player may discard Flying Stallion from hand to draw 2 extra at end of turn.
        // Guarded so it no-ops without the HoA file.
        if (typeof hoaFlyingStallionAmbushReact === "function") {
          await hoaFlyingStallionAmbushReact();
        }
...
function handleVillainEscape(escapedVillain) {
  ...
  if (escapedVillain) {
    ...
    // Heroes of Asgard: an escaping Villain that holds Villainous Weapons hands them to the
    // Mastermind (which then gains their Attack bonuses). Adapts the captive-transfer loops above,
    // retargeting the destination from escapedVillainsDeck to the Mastermind.
    if (
      escapedVillain.capturedWeapons &&
      escapedVillain.capturedWeapons.length > 0 &&
      typeof hoaTransferWeaponsOnEscape === "function"
    ) {
      hoaTransferWeaponsOnEscape(escapedVillain);
    }
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      if (villainCard.name.includes("Master Strike")) {
        ...
        villainCard.type === "Villainous Weapon" &&
        typeof hoaCaptureWeaponOnPlay === "function"
      ) {
        // Heroes of Asgard: a Villainous Weapon revealed from the Villain Deck is NOT a Villain —
        // it is captured by the Villain closest to the Villain Deck (rightmost occupied), or KO'd
        // if the city holds no Villains. It does not enter the city itself. A Weapon carrying its OWN
        // Ambush (The Eternal Flame) resolves capture via that ambush (Surtur / Surtur's Crown
        // reverse-transform), which falls back to the standard rule itself — the weapon-reveal branch
        // bypasses processRegularVillainCard, so its ambushEffect is dispatched here or nowhere.
        if (
          villainCard.ambushEffect &&
          villainCard.ambushEffect !== "None" &&
          typeof window[villainCard.ambushEffect] === "function"
        ) {
          // Valkyrie's Flying Stallion reacts to "an Ambush ability is played, BEFORE it takes effect".
          // Both that trigger and the Ambush keyword are card-type-agnostic, and the rules do not carve
          // out weapon Ambushes — so The Eternal Flame's "Ambush:" opens the same window a Villain's
          // does. Mirrors the call in processRegularVillainCard's ambush branch; no-ops without a
          // Stallion in hand. (Only in the has-an-ambush branch: a plain capture is not an Ambush.)
          if (typeof hoaFlyingStallionAmbushReact === "function") {
            await hoaFlyingStallionAmbushReact();
          }
          await window[villainCard.ambushEffect](villainCard);
        } else {
          await hoaCaptureWeaponOnPlay(villainCard);
        }
      } else {
...
function updateHighlights() {
  ...
  for (let i = 0; i < hq.length; i++) {
    ...
    if (card.type === "Hero") {
      ...
      // HoA Svartalfheim: darkened HQ slots cost +1 Recruit — keep this display gate in sync with the
      // charge in showHeroRecruitButton so the "affordable" highlight matches the true cost. (i is the
      // 0-based HQ slot; hook no-ops outside Svartalfheim.)
      const hoaHQSurcharge =
        typeof hoaHQRecruitSurcharge === "function" ? hoaHQRecruitSurcharge(i) : 0;
      if (totalRecruitPoints + reservedRecruit >= card.cost + hoaHQSurcharge) {
...
if (selectedSchemeEndGame) {
  ...
  for (const condition of endGameConditions) {
    switch (condition) {
        ...
        case "5MoralFailings":
          // Asgardian Test of Worth (Heroes of Asgard): Evil Wins at 5 Moral Failings. Counter lives in
          // expansionHeroesOfAsgard.js (hoaMoralFailings) — incremented in asgardianTestOfWorthTwist.
          if (typeof hoaMoralFailings !== "undefined" && hoaMoralFailings >= 5) {
            finalTwist = true;
            document.getElementById("defeat-context").textContent =
              `Five Moral Failings have been judged against the Heroes. ${mastermind.name} has proven none of them Worthy. All hope is lost.`;
            showDefeatPopup();
          }
          break;

        case "eternalDarknessCoversCity":
          // The Dark World of Svartalfheim (Heroes of Asgard): Evil Wins when all city spaces OR all HQ
          // spaces are covered in Eternal Darkness (derived from the HoA flag arrays, length-guarded).
          if (
            typeof hoaEternalDarknessComplete === "function" &&
            hoaEternalDarknessComplete()
          ) {
            finalTwist = true;
            document.getElementById("defeat-context").textContent =
              `Eternal Darkness has swallowed the battlefield whole. ${mastermind.name} rules a world without light. All hope is lost.`;
            showDefeatPopup();
          }
          break;

        case "5GuardiansDefeated":
          // Ragnarok, Twilight of the Gods (Heroes of Asgard): Evil Wins at 5 Guardians Defeated.
          // Counter lives in expansionHeroesOfAsgard.js (hoaGuardiansDefeated).
          if (typeof hoaGuardiansDefeated !== "undefined" && hoaGuardiansDefeated >= 5) {
            finalTwist = true;
            document.getElementById("defeat-context").textContent =
              `Five Guardians have fallen and the gates of Asgard stand undefended. ${mastermind.name} ushers in the twilight of the gods. All hope is lost.`;
            showDefeatPopup();
          }
          break;

        case "5FrostGiantInvaders":
          // War of the Frost Giants (Heroes of Asgard): Evil Wins at 5 Frost Giant Invaders across the
          // city AND escape pile combined (live count).
          if (
            typeof hoaFrostGiantInvaderCount === "function" &&
            hoaFrostGiantInvaderCount() >= 5
          ) {
            finalTwist = true;
            document.getElementById("defeat-context").textContent =
              `Five Frost Giant Invaders have overrun the realms. ${mastermind.name} leads a frozen host across a shattered world. All hope is lost.`;
            showDefeatPopup();
          }
          break;
...
function updateVillainAttackValues(villain, i) {
  ...
  // Heroes of Asgard — The Dark World of Svartalfheim: a Villain in a CITY space covered by Eternal
  // Darkness gets +1 Attack. CITY-ONLY — the HQ twin (updateHQVillainAttackValues) deliberately OMITS
  // this: HQ Eternal Darkness is a +1 RECRUIT-cost effect (handled in the recruit-cost pipeline), NOT a
  // villain-attack effect. Additive (+=) so it stacks with the villain's own effects; for Svartalfheim
  // no `=` scheme branch above fires, so attackFromScheme is 0 here and += 1 is exact. Off-grid-safe
  // (the hook returns 0 for i < 0). Guarded so it no-ops outside HoA. (No HQ-twin parity by design.)
  if (typeof hoaCityDarknessAttackBonus === "function") {
    villain.attackFromScheme += hoaCityDarknessAttackBonus(i);
  }
  ...
  // Heroes of Asgard: live passive attack (Conqueror / not-Worthy bonus / Victory-Pile scaling —
  // via the card's passiveAbility fn, built in 3c) PLUS the sum of any captured Villainous Weapons'
  // bonuses. One generic delegating hook; all HoA logic lives in expansionHeroesOfAsgard.js. Guarded
  // so it no-ops if that file is absent. Present in BOTH attack twins (updateVillainAttackValues /
  // updateHQVillainAttackValues) — keep identical.
  if (typeof hoaVillainOwnAttackBonus === "function") {
    villain.attackFromOwnEffects += hoaVillainOwnAttackBonus(villain);
  }
...
function updateHQVillainAttackValues(villain) {
  ...
  // Heroes of Asgard — The Dark World of Svartalfheim: the city twin's Eternal-Darkness +1 Attack is
  // INTENTIONALLY ABSENT here. Svartalfheim's HQ darkness is a +1 RECRUIT-cost effect (recruit-cost
  // pipeline), not a villain-attack effect — Villains in a darkened HQ slot get NO attack bonus. Do not
  // add hoaCityDarknessAttackBonus to this twin. (Deliberate asymmetry, not a duplicate-fn omission.)
...
function updateEvilWinsTracker() {
  ...
  switch (selectedScheme.name) {
    ...
    case "Asgardian Test of Worth":
      // Plain-text counter (no markup) → textContent (XSS-safe). HoA counter lives in the expansion file.
      evilWinsText.textContent = `${typeof hoaMoralFailings !== "undefined" ? hoaMoralFailings : 0}/5 Moral Failings`;
      break;

    case "The Dark World of Svartalfheim": {
      const d =
        typeof hoaDarknessCounts === "function"
          ? hoaDarknessCounts()
          : { city: 0, cityTotal: 5, hq: 0, hqTotal: 5 };
      evilWinsText.textContent = `Eternal Darkness — City ${d.city}/${d.cityTotal} · HQ ${d.hq}/${d.hqTotal}`;
      break;
    }

    case "Ragnarok, Twilight of the Gods":
      evilWinsText.textContent = `${typeof hoaGuardiansDefeated !== "undefined" ? hoaGuardiansDefeated : 0}/5 Guardians Defeated`;
      break;

    case "War of the Frost Giants":
      evilWinsText.textContent = `${typeof hoaFrostGiantInvaderCount === "function" ? hoaFrostGiantInvaderCount() : 0}/5 Frost Giant Invaders`;
      break;
...
if (card.temporaryTeleport === true) {
  ...
  // Heroes of Asgard: reset per-turn flags (Royal Decree villain debuff, Usher's armed defeat
  // listener). Flying Stallion's deferred draw is consumed later, in the draw loop below.
  if (typeof hoaResetTurnFlags === "function") hoaResetTurnFlags();
  ...
  // Heroes of Asgard — Valkyrie's Flying Stallion: draw the deferred extra cards on top of the normal
  // hand. Self-clearing (hoaEndTurnExtraDraw returns the count and resets it). Runs AFTER the reset
  // block, so the flag survives to here.
  if (typeof hoaEndTurnExtraDraw === "function") {
    const hoaExtra = hoaEndTurnExtraDraw();
    for (let i = 0; i < hoaExtra; i++) {
      if (!drawOne()) break;
      extraCardsDrawnThisTurn++;
    }
  }
...
function createVillainCopy(villainCard) {
  return {
    ...
    // Heroes of Asgard: the passive-attack marker (Conqueror / not-Worthy / VP-scaling) and any
    // captured Villainous Weapons + their stable bonus must survive the fight copy, or the copy's
    // attack recompute and the on-defeat weapon release see a stripped villain (createVillainCopy
    // whitelist rule — engine-gotchas / CLAUDE.md Villain Card Cloning Gotcha).
    passiveAbility: villainCard.passiveAbility,
    capturedWeapons: villainCard.capturedWeapons ? [...villainCard.capturedWeapons] : undefined,
    weaponAttackBonus: villainCard.weaponAttackBonus
  };
}
...
async function handlePostDefeat(
  ...
  try {
    ...
    // 5b. Heroes of Asgard: captured Villainous Weapons — a defeated holder's Weapons go to your
    // discard pile as 0-cost, classless Artifacts (you get only their written Artifact ability, never
    // the Attack bonus). Mirrors the X-Cutioner release above.
    if (
      Array.isArray(villainCard.capturedWeapons) &&
      villainCard.capturedWeapons.length > 0 &&
      typeof hoaReleaseWeaponsOnDefeat === "function"
    ) {
      try {
        hoaReleaseWeaponsOnDefeat(villainCard);
      } catch (error) {
        console.error("Error releasing captured Villainous Weapons:", error);
      }
    }
    ...
    if (burrowingVillain) {
      ...
      try {
        ...
        // Heroes of Asgard: gainAsArtifact villains (Surtur, Malekith/Hela transforming Tactics) do the
        // same — their fightEffect converts them into an Artifact (Surtur's Crown / a Villainous Weapon)
        // instead of scoring VP. Converters built in 3c/3d.
        if (!villainCard.skrulled && !villainCard.gainAsHero && !villainCard.gainAsArtifact && villainCard.team !== "Infinity Gems") {
          victoryPile.push(villainCard);
          onscreenConsole.log(
...
if (mastermind.name === "Thanos") {
 ...
 if (selectedVillains.includes("Infinity Gems")) {
  ...
  // Heroes of Asgard: Hela's Conqueror passive (via mastermind.passiveAbility, built in 3d) plus any
  // Villainous Weapons the Mastermind captured from an escaping holder. Placed after the name-keyed
  // `=` branches above (Grim Reaper / Hood) and uses += so it never clobbers or is clobbered. Guarded
  // so it no-ops without the HoA file.
  if (typeof hoaMastermindOwnAttackBonus === "function") {
    mastermind.attackFromOwnEffects += hoaMastermindOwnAttackBonus(mastermind);
  }
...
async function handleMastermindPostDefeat(
  ...
  // Heroes of Asgard: a Mastermind holding Villainous Weapons (captured from an escaping holder) hands
  // them to you as Artifacts when you fight it — HoA rules p.2 covers Villains AND Masterminds. Each
  // Mastermind fight defeats one Tactic, so this must fire on the non-final fights, not just the win.
  // Mirrors the villain-side release in handlePostDefeat (step 5b).
  if (typeof hoaReleaseMastermindWeaponsOnDefeat === "function") {
    try {
      hoaReleaseMastermindWeaponsOnDefeat();
    } catch (error) {
      console.error("Error releasing Mastermind-held Villainous Weapons:", error);
    }
  }
...
function showHeroRecruitButton(hqIndex, hero) {
  ...
  // Heroes of Asgard — The Dark World of Svartalfheim: +1 Recruit to recruit a Hero in a darkened HQ
  // slot. Per-SLOT surcharge (hqIndex is 1-based → slot hqIndex-1); both the affordability gate and the
  // charge below read this single `cost`. No-ops (returns 0) outside Svartalfheim / undarkened slots.
  const cost =
    Math.max(0, hero.cost - (hero.infiltrateHQCostReduction || 0)) +
    (typeof hoaHQRecruitSurcharge === "function"
      ? hoaHQRecruitSurcharge(hqIndex - 1)
      : 0);
  const verb =
