...
async function defeatBonuses() {
  ...
  // Heroes of Asgard — Valkyrie's Usher to Valhalla: first VILLAIN defeat this turn (after the
  // superpower armed it) lets you KO a card. Guarded so it no-ops without the HoA file; the flag is
  // only ever set by the superpower, so this is inert otherwise.
  if (typeof hoaUsherDefeatTrigger === "function") {
    await hoaUsherDefeatTrigger(isMastermindDefeat);
  }
}
...
async function drawWound() {
  // Heroes of Asgard — reactive Wound-prevention artifacts (Lady Sif's Winged Helm; Hela's Cloak). If one
  // is in the artifact zone, the player may use it to prevent this Wound (Winged Helm: throw, draw 2;
  // Hela's Cloak: once/turn, draw 1). Both are offered in sequence by hoaOfferWoundPrevention (Cloak
  // first), at most one fires. Checked BEFORE the invulnerability path so every Wound source routes
  // through it. Guarded so it no-ops without the HoA file.
  if (typeof hoaOfferWoundPrevention === "function") {
    const prevented = await hoaOfferWoundPrevention();
    if (prevented) return;
  }
