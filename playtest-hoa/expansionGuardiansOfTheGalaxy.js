...
if (card.name === "Soul Gem" && card.shards && card.shards > 0) {
    ...
    if (card.artifactAbilityUsed === true) {
      ...
      // Heroes of Asgard: a "Thrown Artifact" (keyword) is thrown, not used-in-place — the button
      // reads THROW and routes to hoaThrowArtifact (move to bottom of deck + fire its ability).
      const isThrownArtifact =
        card.keywords && card.keywords.includes("Thrown Artifact");
...
useButton.addEventListener("click", async (e) => {
  ...
  // Heroes of Asgard: Thrown Artifacts leave the zone (to the bottom of the deck) and fire their
  // "when you throw" ability. Handled entirely by hoaThrowArtifact — no artifactAbilityUsed toggle
  // (the card is gone from the zone), and it can be thrown again from the deck on a later turn.
  if (card.keywords && card.keywords.includes("Thrown Artifact") && typeof hoaThrowArtifact === "function") {
    try {
      await closeArtifactsPopup();
      await hoaThrowArtifact(card);
      updateGameBoard();
      if (playerArtifacts.length > 0) openArtifactsPopup();
    } catch (error) {
      console.error(`Error throwing ${card.name}:`, error);
    }
    return;
  }
