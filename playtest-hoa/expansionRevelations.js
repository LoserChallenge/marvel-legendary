...
async function takeOneFromDiscardToHand(sourceName, filterFn = null) {
  updateGameBoard();
  return new Promise((resolve) => {
    // originalIndex is computed against the FULL pile BEFORE filtering, so removal still splices the
    // right card. filterFn (optional) restricts which cards are offered (Heroes of Asgard — The Eternal
    // Flame passes a cost===0 filter). Default (null) = every card, identical to the original behavior.
    const discardCardsWithIndex = playerDiscardPile
      .map((card, index) => ({ ...card, originalIndex: index }))
      .filter((card) => !filterFn || filterFn(card));
    if (discardCardsWithIndex.length === 0) {
