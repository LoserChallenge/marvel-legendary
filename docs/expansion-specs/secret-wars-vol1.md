...
- **RATIFIED (2026-06-24, coordinator) — EITHER/OR.** Teleport is OPTIONAL (SW rulebook p.1: "Instead of playing this card, you MAY set it aside") and MUTUALLY EXCLUSIVE with playing. The +Attack rider resolves ONLY on the play branch; if you Teleport (set aside → rejoins next turn's hand as a bonus card), the rider is SKIPPED. The earlier "teleport this card AND grant" read was wrong; the rule (engine default playOrTeleport) is unchanged.
- **Intended behavior:** Engine default Teleport keyword path. If PLAYED: count Sidekick cards (secondaryType==="Sidekick") played this turn → +1 Attack each. If TELEPORTED: card set aside, no rider.
- **Engine function / pattern:** Teleport keyword handled by the engine (`playOrTeleport()`); the rider is `magikDimensionalPortal` (unconditional, runs on the play branch). Count `otherRealCardsPlayedThisTurn(card)` entries with `secondaryType==="Sidekick"`; grant count × Attack (BOTH `totalAttackPoints` + `cumulativeAttackPoints`); `updateGameBoard()`.
- **Interaction risks:** Count is of OTHER Sidekicks played this turn (Dimensional Portal isn't a Sidekick). Rider fires on the play branch only — teleporting skips it.
- **Executable assertion:** Set up: play 2 Sidekick cards this turn → play Dimensional Portal and choose PLAY → assert current-turn Attack +2 (rider fires, card NOT teleported). Second run: choose TELEPORT → assert the card is set aside (rejoins next turn's hand) and NO Attack rider fires.
...
- **RATIFIED (2026-06-24, coordinator) — EITHER/OR.** Teleport is OPTIONAL and MUTUALLY EXCLUSIVE with playing; the choose-VP-villain rider resolves ONLY on the play branch. Teleporting (set aside → next turn's hand) SKIPS the rider. The earlier "teleport AND grant" read was wrong.
- **Intended behavior:** Engine default Teleport keyword path. If PLAYED: choose one Villain or Mastermind in your Victory Pile; gain Attack equal to its printed VP (`victoryPoints` field). If TELEPORTED: card set aside, no rider. Empty eligible pile → no Attack.
- **Engine function / pattern:** Teleport keyword handled by the engine; the rider is `magikWieldTheSoulsword` (unconditional, play branch). `showCardSelectionPopup` over Victory-Pile entries of type Villain or Mastermind; grant chosen `victoryPoints` Attack (BOTH `totalAttackPoints` + `cumulativeAttackPoints`); `updateGameBoard()`.
- **Interaction risks:** Victory Pile may contain Bystanders/Heroes/Tactics — filter to Villain/Mastermind only. "Printed VP" = the card's `victoryPoints` (mastermind/tactic VP usually 6; ascended villains revert to VP-6 villains). Empty-eligible-pile guard. Rider on play branch only.
- **Executable assertion:** Set up: Victory Pile containing a VP-4 Villain → play Wield the Soulsword and choose PLAY → choose that Villain → assert current-turn Attack +4 (card NOT teleported). Teleport branch → set aside, no Attack. Run with no Villain/Mastermind in the pile → assert no Attack granted.
...
- **RATIFIED (2026-06-24, coordinator) — EITHER/OR (corrects the earlier "teleports regardless" read).** Teleport is OPTIONAL and MUTUALLY EXCLUSIVE with playing — the card does NOT self-teleport. The whole "may KO a Bystander → +2 Attack" rider lives on the PLAY branch. The inner "may KO" is a SECOND nested 'may' (on the play branch), distinct from Teleport's own optional choice. If you TELEPORT (set aside → next turn's hand), the entire rider (KO + Attack) is SKIPPED.
- **Intended behavior:** Engine default Teleport keyword path. If PLAYED: OPTIONAL — may KO one Bystander from your own Victory Pile; "If you do" → +2 Attack; declining → no Attack. (KOing a Bystander from your Victory Pile costs 1 VP — intended.) If TELEPORTED: card set aside, no KO, no Attack.
- **Engine function / pattern:** Teleport keyword handled by the engine; the rider is `thanosTransdimensionalOverlord` (unconditional, play branch). `showHeroAbilityMayPopup` yes/no gate; if multiple Bystanders, `showCardSelectionPopup` to pick. On KO: identity-splice from `victoryPile` → `koPile`, grant +2 Attack (BOTH `totalAttackPoints` + `cumulativeAttackPoints`).
- **Interaction risks:** Rider on the play branch only — teleporting skips it. KO from Victory Pile reduces VP. "If you do" gates the +2 Attack. Must touch cumulativeAttackPoints.
- **Executable assertion:** Set up: player's Victory Pile contains a Bystander → play Transdimensional Overlord and choose PLAY → accept the inner KO → assert the Bystander is in the KO pile and +2 Attack to both totalAttackPoints and cumulativeAttackPoints (card is PLAYED, NOT teleported). Re-run, choose PLAY then decline the inner KO → no KO, no +2. Separately, choose TELEPORT → assert the card is set aside (next turn's hand) and the whole rider (KO + Attack) is skipped.
