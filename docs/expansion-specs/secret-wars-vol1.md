...
- **Intended behavior:** Special ability is an "each other player" effect → in 1-player solo there ARE no other players. Per the non-Location "each other player" precedent (CLAUDE.md rule 8; solo decisions table), this resolves to self-apply OR no-op — must confirm at build. SELF-APPLY reading: the active (only) player reveals a [TECH] Hero or chooses a Henchman from their OWN Victory Pile; if they choose a Henchman, "you defeat all those Henchmen for free" defeats it (Fight effect fires). NO-OP reading: no other players → no Henchmen surrendered → the free-defeat clause has nothing to act on. Superpower [CABAL]: +1 Attack per Henchman the player has defeated THIS TURN (counts all henchman defeats this turn, not only ones from this card) — updates totalAttackPoints + cumulativeAttackPoints.
...
- **Executable assertion:** Set up state S: solo (no other players), player's Victory Pile has one Henchman villain, player has a [CABAL] + [TECH] Hero this turn. → resolve Inhuman Mastery special, then its superpower → (self-apply build) assert the chosen Henchman is defeated for free (Victory Pile/again-VP per defeat path) and the [CABAL] superpower adds +1 Attack per Henchman defeated this turn to BOTH totalAttackPoints and cumulativeAttackPoints. (no-op build) assert special does nothing and superpower still tallies henchmen defeated by other means this turn.
- **LOW reason:** "each other player" in solo (self-apply vs no-op) is genuinely unresolved per the mechanics doc / solo decisions table — confirm exact handling at build; mandatory in-game /game-test.
...
- **Intended behavior:** Superpower (requires a [RANGED] Hero this turn). "Each other player" effect → in 1-player solo, no other players. Per the non-Location "each other player" precedent (solo decisions table), resolve to self-apply OR no-op — confirm at build. SELF-APPLY reading: the active player reveals a [RANGED] Hero OR chooses a Bystander from their own Victory Pile; if a Bystander is surrendered, Thanos's player "rescues" it (gains its rescue benefit). NO-OP reading: no other players to surrender Bystanders → nothing to rescue. Default to the established precedent at build.
...
- **Executable assertion:** Set up state S (solo): player's Victory Pile has a Bystander, [RANGED] Hero this turn. → trigger superpower → (self-apply build) assert the player may reveal a [RANGED] Hero to avoid surrendering, or surrender the Bystander and rescue it (rescue benefit applied); (no-op build) assert nothing happens. Confirm which at build.
...
### Manhattan — Ultimate Captain America  (confidence: LOW)
...
- **Intended behavior:** On Fight, convert to Hero form: **Team Avengers, Class Strength, base 0+ Attack**, ability **"You get +1 Attack for each color of Hero you have"** (count distinct Hero colors/classes the player has in play/hand this turn → that many +Attack). Cost as a Hero = 6 (old villain Attack). Skip VP push; route to discard.
- **Engine function / pattern:** `ultimateCaptainAmericaFight(villainCard)` → `gainScarletWitchAsHero`-style converter parameterized on this Hero form (set `team`, `classes:["Strength"]`, base attack, `cost:6`, install the per-color-Attack ability as the card's hero ability for when it is later played). Reuse the `skrulled`-style VP-bypass flag.
- **Interaction risks:** **gain-as-hero must SKIP the VP push** (these have no VP — a stray push grants phantom VP). The "+1 Attack per color of Hero you have" ability resolves only when the card is later PLAYED as a Hero (not at gain time) — make sure the installed ability is the conditional/unconditional hero ability, not run at conversion. Whitelist gap = stripped copy converts wrong. Verify "color of Hero" maps to the class→color table (Strength=Green, Instinct=Yellow, Covert=Red, Tech=Black, Range=Blue).
- **LOW confidence — why:** new inverted-Skrull pattern (gain villain→hero with a hero ability that activates on later play); "+1 Attack per color of Hero you have" needs a distinct-color count helper that may not exist.
- **Executable assertion:** Fight Ultimate Captain America → assert it is NOT in the Victory Pile, IS in `playerDiscardPile`, now `type:"Hero"`, `team:"Avengers"`, `classes` includes "Strength", `cost === 6`, and carries the per-color ability. Later play it with Heroes of 3 distinct colors in play → assert +3 Attack.
