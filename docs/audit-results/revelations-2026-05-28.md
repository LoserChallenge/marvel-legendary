...
- **GP-3d (bystander-capture + hero-KO):** `captureBystanderFromVPToLocation` (#9 Carnival of Wonders, leans on PT-2) + adapt `koUpToNHeroesYouHave` (#10 Dragon of Heaven). ✅ **DONE + VERIFIED (commit `c0f7f5a`).** #9 → new thin `captureBystanderFromVPToLocation(locationCard, label)` wrapping the shipped `moveOneFromVictoryPile` (VP Bystander picker → `locationCard.capturedBystanders`; rescued on Location defeat via PT-2, `script.js:12084`); trigger now `carnivalOfWondersTrigger(locationCard, cityIndex)`. #10 → new `koNonGreyHeroFromHand` + new `removeOneFromHand` picker (hand twin of `moveOneFromVictoryPile`: 0→nothing/1→auto/2+→single-select; `koBonuses()` per KO). **Rule-8 decision (diverges from catalog's prior "heroes-you-have" lean): KO source = HAND ONLY.** Basis — the "reveals their hand and KOs one of their non-grey Heroes" idiom scopes the KO to the revealed hand, exactly matching sibling card Cult of Skulls #5 ("reveals their hand and discards a non-grey card", already hand-scoped). The card's *own* fight effect ("KO up to two of your Heroes", no reveal-hand prefix) correctly stays broader (heroes-you-have) — distinct wording, distinct scope. Rules PDF inaccessible from worktree (gitignored 105MB, main-folder only); inventory + sibling-construction precedent decisive, not escalated. Rule-7 gate: **expansion-validator 7/7** (resolve-once on all branches incl. defensive DOM-absent fallback; awaits correct; `koBonuses` sync); **cold-read code review clean** (verified all globals resolve — no `escapePile`-class undefined; `koBonuses` reads `koPile[len-1]` and push-before-postAction ordering correct; `handIndex`/`vpIndex` captured-at-build vs spliced-at-confirm is sort-order-safe; grey-SHIELD + Wound exclusion correct; PT-2 rescue contract intact); **dual-mode live `/game-test` vs FRESH worktree source (clean port 8766, GP-3d symbols confirmed loaded)** — Dragon 0/1/2+ (grey SHIELD + Wound never KO-eligible; picker shows only non-grey Heroes, KOs the SELECTED card by preserved index, promise resolves), Carnival 0/1/2+ (picker shows only Bystandes, captures the SELECTED bystander onto `locationCard.capturedBystanders`, removes it from VP, resolves); What If? parity identical. Only console errors = benign `sw.js`/x.svg 404s. (The `updateGameBoard` crash on minimal injection is the known harness artifact — logic runs before the trailing re-render; stubbed for clean runs.)
