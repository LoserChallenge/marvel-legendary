...
- **PT-3 — HYDRA transforming Scheme, S.H.I.E.L.D. Officer stack** [feature gap]: (a) no on-screen count of Officers stacked by the Scheme; (b) no "pay 3 Recruit → gain a stacked Officer" player action.
  ...
  - **CURRENT STATE:**
    ...
    - Evil-wins: only the `>= 15` branch exists (`script.js:9569` + a duplicate `>= 15` log at `expansionRevelations.js:2527`); the DB declares `evilWinsCondition:"hydraInfiltrationComplete"` (`cardDatabase.js:1591`). **"Stack runs out" is UNWIRED** (no `shieldDeck.length === 0` / stack-empty check anywhere in the loss path — the only `shieldDeck.length===0` guards are inside `recruitOfficer`).
