...
- **Inventory verification and the DB↔inventory linter check DIFFERENT links in the chain — you need both.** The 3-pass inventory verification (image ↔ inventory) confirms *"does the inventory match the physical card."* The `tools/lint-card-data.js` content gate (DB ↔ inventory) confirms *"does the game's `cardDatabase.js` match the inventory."* A correct inventory does NOT imply a correct DB: Proxima Midnight "Supernova Spear" (B22) had the right value in the inventory (all 3 passes clean) but `recruit: 0` in the DB — only the linter caught it. Run the linter green as the Phase-1 content gate; passing inventory verification is not a substitute.
