...
- SUPERPOWER: [X-MEN]: Choose a Villain in the city. You get **+**Attack equal to its VP, usable only against other Villains or the Mastermind. *(Pass 3 confirmed against physical card: trigger is [X-MEN] — the reference was right; the Pass-1/2 image read of "Covert" was wrong.)*
...
1. **✅ RESOLVED (Pass 3) — Karma "Control Like a Puppet" superpower trigger = [X-MEN].** Pass 2 had kept the image read [COVERT]; the physical card confirms the trigger is **X-MEN**, matching the reference. The Pass-1/2 image read was wrong — a reminder that even an icon read that looks "pixel-identical" to a known icon can mislead, and the reference wins. Effect line updated to [X-MEN].
