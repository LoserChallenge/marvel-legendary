# Core Set

## Heroes

<!-- 15 heroes, 4 cards each = 60 cards total -->
<!-- Data source: cardDatabase.js (structured fields) + card images (effect text only) -->

### Black Widow

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Mission Accomplished | Tech | Avengers | 2 | 0 | "Draw a card. [Tech]: Rescue a Bystander." |
| Dangerous Rescue | Covert | Avengers | 3 | 2 [Attack] | "[Covert]: You may KO a card from your hand or discard pile. If you do, rescue a Bystander." |
| Covert Operation | Covert | Avengers | 4 | 0+ [Attack] | "You get +1 [Attack] for each Bystander in your Victory Pile." |
| Silent Sniper | Covert | Avengers | 7 | 4 [Attack] | "Defeat a Villain or Mastermind that has a Bystander." |

### Captain America

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Avengers Assemble! | Instinct | Avengers | 3 | 0 [Recruit] | "You get +1 [Recruit] for each color of Hero you have." |
| Perfect Teamwork | Strength | Avengers | 4 | 0 [Attack] | "You get +1 [Attack] for each color of Hero you have." |
| Diving Block | Tech | Avengers | 6 | 4 [Attack] | "If you would gain a Wound, you may reveal this card and draw a card instead." |
| A Day Unlike Any Other | Covert | Avengers | 7 | 0+ [Attack] | "[Avengers]: You get +3 [Attack] for each other [Avengers] Hero you played this turn." |

### Cyclops

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Optic Blast | Range | X-Men | 3 | 3 [Attack] | "To play this card, you must discard a card from your hand." |
| Determination | Strength | X-Men | 2 | 3 [Recruit] | "To play this card, you must discard a card from your hand." |
| Unending Energy | Range | X-Men | 6 | 4 [Attack] | "If a card effect makes you discard this card, you may return this card to your hand." |
| X-Men United | Range | X-Men | 8 | 0+ [Attack] | "[X-Men]: You get +2 [Attack] for each other [X-Men] Hero you played this turn." |

### Deadpool

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Here, Hold This For a Second | Tech | None | 3 | 2 [Recruit] | "A Villain of your choice captures a Bystander." |
| Oddball | Covert | None | 5 | 0+ [Attack] | "You get +1 [Attack] for each other Hero with an odd-numbered [Cost] you played this turn." |
| Hey, Can I Get a Do-Over? | Instinct | None | 3 | 2 [Attack] | "If this is the first Hero you played this turn, you may discard the rest of your hand and draw four cards." |
| Random Acts of Unkindness | Instinct | None | 7 | 6 [Attack] | "You may gain a Wound to your hand. Then each player passes a card from their hand to the player on their left." |

### Emma Frost

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Mental Discipline | Range | X-Men | 3 | 1 [Recruit] | "Draw a card." |
| Shadowed Thoughts | Covert | X-Men | 4 | 0+ [Attack] | "[Covert]: You may play the top card of the Villain Deck. If you do, you get +2 [Attack]." |
| Psychic Link | Instinct | X-Men | 5 | 3 [Attack] | "Each player may reveal another [X-Men] Hero. Each player who does draws a card." |
| Diamond Form | Strength | X-Men | 7 | 5 [Attack], 0 [Recruit] | "Whenever you defeat a Villain or Mastermind this turn, you get +3 [Recruit]." |

### Gambit

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Stack the Deck | Covert | X-Men | 2 | 0 | "Draw two cards. Then put a card from your hand on top of your deck." |
| Card Shark | Range | X-Men | 4 | 2 [Attack] | "Reveal the top card of your deck. If it's an [X-Men] Hero, draw it." |
| Hypnotic Charm | Instinct | X-Men | 3 | 2 [Recruit] | "Reveal the top card of your deck. Discard it or put it back. [Instinct]: Do the same thing to each other player's deck." |
| High Stakes Jackpot | Instinct | X-Men | 7 | 4 [Attack] | "Reveal the top card of your deck. You get +[Attack] equal to that card's cost." |

### Hawkeye

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Quick Draw | Instinct | Avengers | 3 | 1 [Attack] | "Draw a card." |
| Team Player | Tech | Avengers | 4 | 0+ [Attack] | "[Avengers]: You get +1 [Attack]." |
| Covering Fire | Tech | Avengers | 5 | 3 [Attack] | "[Tech]: Choose one: each other player draws a card or each other player discards a card." |
| Impossible Trick Shot | Tech | Avengers | 7 | 5 [Attack] | "Whenever you defeat a Villain or Mastermind this turn, rescue three Bystanders." |

### Hulk

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Unstoppable Hulk | Instinct | Avengers | 4 | 2 [Attack] | "You may KO a Wound from your hand or discard pile. If you do, you get +2 [Attack]." |
| Growing Anger | Strength | Avengers | 3 | 0+ [Attack] | "[Strength]: You get +1 [Attack]." |
| Crazed Rampage | Strength | Avengers | 5 | 4 [Attack] | "Each player gains a Wound." |
| Hulk Smash! | Strength | Avengers | 8 | 0+ [Attack] | "[Strength]: You get +5 [Attack]." |

### Iron Man

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Endless Invention | Tech | Avengers | 3 | 0 | "Draw a card. [Tech]: Draw another card." |
| Repulsor Rays | Range | Avengers | 3 | 0+ [Attack] | "[Ranged]: You get +1 [Attack]." |
| Arc Reactor | Tech | Avengers | 5 | 0+ [Attack] | "[Tech]: You get +1 [Attack] for each other [Tech] Hero you played this turn." |
| Quantum Breakthrough | Tech | Avengers | 7 | 0 | "Draw two cards. [Tech]: Draw two more cards." |

### Nick Fury

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Battlefield Promotion | Covert | S.H.I.E.L.D. | 4 | 0 | "You may KO a [S.H.I.E.L.D.] Hero from your hand or discard pile. If you do, you may gain a S.H.I.E.L.D. Officer to your hand." |
| High-Tech Weaponry | Tech | S.H.I.E.L.D. | 3 | 0+ [Attack] | "[Tech]: You get +1 [Attack]." |
| Legendary Commander | Strength | S.H.I.E.L.D. | 6 | 0+ [Attack] | "You get +1 [Attack] for each other [S.H.I.E.L.D.] Hero you played this turn." |
| Pure Fury | Tech | S.H.I.E.L.D. | 8 | 0 | "Defeat any Villain or Mastermind whose [Attack] is less than the number of [S.H.I.E.L.D.] Heroes in the KO pile." |

### Rogue

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Energy Drain | Covert | X-Men | 3 | 0+ [Recruit] | "[Covert]: You may KO a card from your hand or discard pile. If you do, you get +1 [Recruit]." |
| Borrowed Brawn | Strength | X-Men | 4 | 0+ [Attack] | "[Strength]: You get +3 [Attack]." |
| Copy Powers | Covert | X-Men | 5 | 0 | "Play this card as a copy of another Hero you played this turn. This card is both [Covert] and the color you copy." |
| Steal Abilities | Strength | X-Men | 8 | 4 [Attack] | "Each player discards the top card of their deck. Play a copy of each of those cards." |

### Spider-Man

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Astonishing Strength | Strength | Spider Friends | 2 | 1 [Recruit] | "Reveal the top card of your deck. If that card costs 2 or less, draw it." |
| Great Responsibility | Instinct | Spider Friends | 2 | 1 [Attack] | "Reveal the top card of your deck. If that card costs 2 or less, draw it." |
| Web-Shooters | Tech | Spider Friends | 2 | 0 | "Rescue a Bystander. Reveal the top card of your deck. If that card costs 2 or less, draw it." |
| The Amazing Spider-Man | Covert | Spider Friends | 2 | 0 | "Reveal the top three cards of your deck. Put any that cost 2 or less into your hand. Put the rest back in any order." |

### Storm

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Lightning Bolt | Range | X-Men | 4 | 2 [Attack] | "Any Villain you fight on the Rooftops this turn gets -2 [Attack]." |
| Gathering Stormclouds | Range | X-Men | 3 | 2 [Recruit] | "[Ranged]: Draw a card." |
| Spinning Cyclone | Covert | X-Men | 6 | 4 [Attack] | "You may move a Villain to a new city space. Rescue any Bystanders captured by that Villain. (If you move a Villain to a city space that already has a Villain, swap them.)" |
| Tidal Wave | Range | X-Men | 7 | 5 [Attack] | "Any Villain you fight on the Bridge this turn gets -2 [Attack]. [Ranged]: The Mastermind gets -2 [Attack] this turn." |

### Thor

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Surge of Power | Range | Avengers | 4 | 0 [Attack], 2 [Recruit] | "If you made 8 or more [Recruit] this turn, you get +3 [Attack]." |
| Odinson | Strength | Avengers | 3 | 0+ [Recruit] | "[Strength]: You get +2 [Recruit]." |
| Call Lightning | Range | Avengers | 6 | 0+ [Attack] | "[Ranged]: You get +3 [Attack]." |
| God of Thunder | Range | Avengers | 8 | 0 [Attack], 5 [Recruit] | "You can use [Recruit] as [Attack] this turn." |

### Wolverine

| Card Name | Class | Team | Cost | Base Value | Effect Text |
|-----------|-------|------|------|------------|-------------|
| Healing Factor | Instinct | X-Men | 3 | 2 [Attack] | "You may KO a Wound from your hand or discard pile. If you do, draw a card." |
| Keen Senses | Instinct | X-Men | 2 | 1 [Attack] | "[Instinct]: Draw a card." |
| Frenzied Slashing | Instinct | X-Men | 5 | 2 [Attack] | "[Instinct]: Draw two cards." |
| Berserker Rage | Instinct | X-Men | 8 | 0+ [Attack] | "Draw three cards. [Instinct]: You get +1 [Attack] for each extra card you've drawn this turn." |

## Villains

<!-- Data source: cardDatabase.js villains array, IDs 1-7 -->

### Brotherhood

| Villain Name | Group | Attack | VP | Fight Effect | Escape Effect | Ambush |
|-------------|-------|--------|-----|-------------|---------------|--------|
| Blob | Brotherhood | 4 | 2 | — | — | — |
| Juggernaut | Brotherhood | 6 | 4 | — | KO two Heroes from your hand | KO two Heroes from your discard pile |
| Mystique | Brotherhood | 5 | 3 | — | Transforms into a Scheme Twist placed on top of the Villain Deck, then that card is played | — |
| Sabretooth | Brotherhood | 5 | 3 | Reveal an X-Men Hero or gain a Wound | Reveal an X-Men Hero or gain a Wound | — |

**Notes:** Blob has a fight condition requiring an X-Men Hero you have.

### Enemies of Asgard

| Villain Name | Group | Attack | VP | Fight Effect | Escape Effect | Ambush |
|-------------|-------|--------|-----|-------------|---------------|--------|
| Frost Giant (x3) | Enemies of Asgard | 4 | 2 | Reveal a [Ranged] Hero or gain a Wound | Reveal a [Ranged] Hero or gain a Wound | — |
| Destroyer (x1) | Enemies of Asgard | 7 | 5 | KO all your S.H.I.E.L.D. Heroes | KO two of your Heroes | — |
| Enchantress (x2) | Enemies of Asgard | 6 | 4 | Draw three cards | — | — |
| Ymir, Frost Giant King (x2) | Enemies of Asgard | 6 | 4 | KO any number of Wounds from your hand or discard pile | — | Reveal a [Ranged] Hero or gain a Wound |

### HYDRA

| Villain Name | Group | Attack | VP | Fight Effect | Escape Effect | Ambush |
|-------------|-------|--------|-----|-------------|---------------|--------|
| Viper (x1) | HYDRA | 5 | 3 | Reveal another HYDRA Villain in your Victory Pile or gain a Wound | Reveal another HYDRA Villain in your Victory Pile or gain a Wound | — |
| Supreme HYDRA (x1) | HYDRA | 6 | 3 | — | — | — |
| Endless Armies of HYDRA (x3) | HYDRA | 4 | 3 | — | — | — |
| HYDRA Kidnappers (x3) | HYDRA | 3 | 1 | You may gain a S.H.I.E.L.D. Officer | — | — |

### Masters of Evil

| Villain Name | Group | Attack | VP | Fight Effect | Escape Effect | Ambush |
|-------------|-------|--------|-----|-------------|---------------|--------|
| Ultron | Masters of Evil | 6 | 2 | — | Reveal a [Tech] Hero or gain a Wound | — |
| Whirlwind | Masters of Evil | 4 | 2 | If fought on Rooftops or Bridge, KO two of your Heroes; otherwise no effect | — | — |
| Baron Zemo | Masters of Evil | 6 | 4 | For each of your Avengers Heroes, rescue a Bystander | — | — |
| Melter | Masters of Evil | 5 | 3 | Reveal the top card of your deck; choose to KO it or put it back | — | — |

### Radiation

| Villain Name | Group | Attack | VP | Fight Effect | Escape Effect | Ambush |
|-------------|-------|--------|-----|-------------|---------------|--------|
| Abomination | Radiation | 5 | 3 | If fought on Streets or Bridge, rescue 3 Bystanders; otherwise no effect | — | — |
| The Leader | Radiation | 4 | 2 | — | — | Play the top card of the Villain Deck |
| Maestro | Radiation | 6 | 4 | For each of your [Strength] Heroes, KO one of your Heroes | — | — |
| Zzzax | Radiation | 5 | 3 | Reveal a [Strength] Hero or gain a Wound | Reveal a [Strength] Hero or gain a Wound | — |

### Skrulls

| Villain Name | Group | Attack | VP | Fight Effect | Escape Effect | Ambush |
|-------------|-------|--------|-----|-------------|---------------|--------|
| Super-Skrull (x3) | Skrulls | 4 | 2 | KO one of your Heroes | — | — |
| Skrull Shapeshifters (x3) | Skrulls | 0* | 2 | Gain the captured Hero | Escapes with captured Hero (Hero is lost) | Captures the rightmost Hero from HQ; this Villain's attack equals the captured Hero's cost |
| Skrull Queen Veranke (x1) | Skrulls | 0* | 4 | Gain the captured Hero | Escapes with captured Hero (Hero is lost) | Captures the highest-cost Hero from HQ; this Villain's attack equals the captured Hero's cost |
| Paibok the Power Skrull (x1) | Skrulls | 8 | 3 | Choose a Hero in the HQ to gain for free | — | — |

**Notes:** Skrull Shapeshifters and Skrull Queen Veranke have 0 base attack; their actual attack equals the cost of the Hero they captured.

### Spider-Foes

| Villain Name | Group | Attack | VP | Fight Effect | Escape Effect | Ambush |
|-------------|-------|--------|-----|-------------|---------------|--------|
| Venom | Spider-Foes | 5 | 3 | — | Gain a Wound | — |
| Green Goblin | Spider-Foes | 6 | 4 | — | — | A Bystander is captured by the Villain entering the Sewers |
| The Lizard | Spider-Foes | 3 | 2 | If fought in the Sewers, gain a Wound; otherwise no effect | — | — |
| Doctor Octopus | Spider-Foes | 4 | 2 | Draw 8 cards instead of 6 next turn (draw 2 extra cards next turn) | — | — |

**Notes:** Venom has a fight condition requiring a [Covert] Hero you have.

## Masterminds

<!-- Data source: cardDatabase.js masterminds array, IDs 1-4 -->

### Dr. Doom

| Field | Value |
|-------|-------|
| Strength | 12 (9 + 3) |
| Always Leads | Doombot Legion (henchmen) |
| Master Strike | Each player with exactly six cards in hand reveals a [Tech] Hero or puts two cards from their hand on top of their deck. |

**Tactics:**

| Tactic Name | VP | Effect Text |
|-------------|-----|-------------|
| Treasures of Latveria | 5 | When you draw a new hand of cards at the end of this turn, draw three extra cards. |
| Dark Technology | 5 | You may recruit a [Tech] or [Ranged] Hero from the HQ for free. |
| Secrets of Time Travel | 5 | Take another turn after this one. |
| Monarch's Decree | 5 | Choose one: each other player draws a card or each other player discards a card. |

### Loki

| Field | Value |
|-------|-------|
| Strength | 12 (10 + 2) |
| Always Leads | Enemies of Asgard (villain) |
| Master Strike | Each player reveals a [Strength] Hero or gains a Wound. |

**Tactics:**

| Tactic Name | VP | Effect Text |
|-------------|-----|-------------|
| Cruel Ruler | 5 | Defeat a Villain in the City for free. |
| Maniacal Tyrant | 5 | KO up to four cards from your discard pile. |
| Vanishing Illusions | 5 | Each other player KOs a Villain from their Victory Pile. |
| Whispers and Lies | 5 | Each other player KOs two Bystanders from their Victory Pile. |

### Red Skull

| Field | Value |
|-------|-------|
| Strength | 8 (7 + 1) |
| Always Leads | HYDRA (villain) |
| Master Strike | Each player KOs a Hero from their hand. |

**Tactics:**

| Tactic Name | VP | Effect Text |
|-------------|-----|-------------|
| Ruthless Dictator | 5 | Look at the top three cards of your deck. KO one, discard one and put one back on top of your deck. |
| Endless Resources | 5 | You gain +4 [Recruit]. |
| Negablast Grenades | 5 | You gain +3 [Attack]. |
| HYDRA Conspiracy | 5 | Draw two cards. Then draw another card for each HYDRA Villain in your Victory Pile. |

### Magneto

| Field | Value |
|-------|-------|
| Strength | 10 (8 + 2) |
| Always Leads | Brotherhood (villain) |
| Master Strike | Each player reveals an X-Men Hero or discards down to four cards. |

**Tactics:**

| Tactic Name | VP | Effect Text |
|-------------|-----|-------------|
| Bitter Captor | 5 | Recruit an X-Men Hero from the HQ for free. |
| Crushing Shockwave | 5 | Each other player reveals an X-Men Hero or gains two Wounds. |
| Electromagnetic Bubble | 5 | Choose one of your X-Men Heroes. When you draw a new hand of cards at the end of this turn, add that Hero to your hand as a seventh card. |
| Xavier's Nemesis | 5 | For each of your X-Men Heroes, rescue a Bystander. |

## Schemes

<!-- Data source: cardDatabase.js schemes array, IDs 1-8 -->

### Midtown Bank Robbery

| Field | Value |
|-------|-------|
| Bystanders | 12 |
| Twists | 8 |
| End Game | 8 Bystanders carried away |
| Twist Effect | Any Villain in the Bank captures 2 Bystanders. Then play the top card of the Villain Deck. |

### Negative Zone Prison Breakout

| Field | Value |
|-------|-------|
| Bystanders | 1 |
| Twists | 8 |
| End Game | 12 Villains escape |
| Twist Effect | Play the top two cards of the Villain Deck. |

### Portals to the Dark Dimension

| Field | Value |
|-------|-------|
| Bystanders | 1 |
| Twists | 7 |
| End Game | 7th Twist triggers Evil Wins |
| Twist Effect | (variable, see below) |

**Twist sequence:**

| Twist # | Effect |
|---------|--------|
| 1 | A Dark Portal opens above the Mastermind. The Mastermind gains +1 [Attack]. |
| 2 | A Dark Portal opens above the Bridge. Villains in this city space gain +1 [Attack]. |
| 3 | A Dark Portal opens above the Streets. Villains in this city space gain +1 [Attack]. |
| 4 | A Dark Portal opens above the Rooftops. Villains in this city space gain +1 [Attack]. |
| 5 | A Dark Portal opens above the Bank. Villains in this city space gain +1 [Attack]. |
| 6 | A Dark Portal opens above the Sewers. Villains in this city space gain +1 [Attack]. |
| 7 | Evil Wins! |
| 8+ | Extra Twist. No effect. |

### Replace Earth's Leaders with Killbots

| Field | Value |
|-------|-------|
| Bystanders | 0 |
| Twists | 5 |
| End Game | 5 Killbots in city |
| Twist Effect | Killbots gain +1 [Attack]. |

### Secret Invasion of the Skrull Shapeshifters

| Field | Value |
|-------|-------|
| Bystanders | 1 |
| Twists | 8 |
| End Game | 6 escaped Skrull Heroes |
| Twist Effect | The highest-cost Hero from the HQ moves into the Sewers as a Skrull Villain. |

**Notes:** Requires the Skrulls villain group. Uses 6 heroes instead of the normal count.

### Superhero Civil War

| Field | Value |
|-------|-------|
| Bystanders | 1 |
| Twists | 8 |
| End Game | Hero deck empty |
| Twist Effect | KO all the Heroes in the HQ. |

**Notes:** Uses 4 heroes instead of the normal count.

### The Legacy Virus

| Field | Value |
|-------|-------|
| Bystanders | 1 |
| Twists | 8 |
| End Game | Wound deck empty |
| Twist Effect | Reveal a [Tech] Hero or gain a Wound. |

### Unleash the Power of the Cosmic Cube

| Field | Value |
|-------|-------|
| Bystanders | 1 |
| Twists | 8 |
| End Game | 8th Twist triggers Evil Wins |
| Twist Effect | (variable, see below) |

**Twist sequence:**

| Twist # | Effect |
|---------|--------|
| 1-4 | Nothing happens yet... |
| 5-6 | You gain a Wound. |
| 7 | You gain three Wounds. |
| 8 | Evil Wins! |
| 9+ | Extra Twist. No effect. |

## Henchmen

<!-- Data source: cardDatabase.js henchmen array, IDs 1-4 -->

| Henchmen Group | Attack | VP | Fight Effect |
|---------------|--------|-----|-------------|
| Doombot Legion | 3 | 1 | Reveal the top two cards of your deck; KO one and put the other back on top |
| Hand Ninjas | 3 | 1 | +1 [Recruit] |
| Savage Land Mutates | 3 | 1 | Draw one extra card next turn |
| Sentinel | 3 | 1 | KO one of your Heroes (from hand, played cards, or artifacts) |

