...
- **2026-06-21 — Debug `console.log` cleanup.** Removed 589 leftover debug traces from `script.js`, `cardAbilities*.js`, `expansion*.js` (pure deletion; kept `console.error`/`warn` + the console-capture harness). Verified via `node --check` all files + Playwright smoke test (game init + a turn, zero runtime errors). Resolves the prior "red flag rule vs reality" divergence. Optional follow-on (not done): a guard hook flagging *new* `console.log` in game logic — now viable since the baseline is clean.
