...
| heroes-of-asgard | A | ✅ | ✅ 2026-04-04 | ✅ 2026-04-04 | ✅ 2026-04-04 | ✅ | ✅ 2026-07-04 | — |
...
`/analyze-expansion` complete (2026-07-04). Mechanics reference: `docs/expansion-mechanics/heroes-of-asgard.md` — reuse verdicts filled by `pattern-reuse-scout` (~80–85% reuse; nothing built from scratch — Artifact zone/bystander-attach/transform-and-defer-VP/attack-modifier pipeline all reused). **Solo rulings resolved:** The Mangog + Naglfar "player on your right" → **self-apply to the solo player** (verified vs the official Advanced Solo "do it yourself" rule). All 4 schemes confirmed solo-viable (no SKIP/DEFER). One reuse-scout catch: "closest to the Villain Deck" = the rightmost (Sewers/entry) city slot, not index 0. **Next: `/new-expansion`** (cuts the `heroes-of-asgard` worktree branch).
