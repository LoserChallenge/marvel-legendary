...
> **SUPERSEDED 2026-07-03 (same day, later work):** This BLOCKER is CLOSED. The A2 pass/fail contract was built and merged to local master — `gtRunTestCase()` runs an "A2 TestCase" and emits a fail-closed PASS/FAIL/ERROR/BLOCKED verdict (`.claude/skills/game-test/SKILL.md`, "Executor CORE"; design spec `docs/superpowers/specs/2026-07-03-gametest-assertion-contract-design.md`). See `docs/automation-initiative.md` §2. The "verified absent" finding below reflects the state *before* that same-day harness build.
...
- **Sibling selection popups extended 2026-07-03:** `installGameTestHarness()` now drives all genuine selection popups via a registry — `card-choice`, `card-choice-city-hq`, `order-choice` (verified live on order-choice; no card-choice regression). `draw-choice`/`final-blow` verified YES/NO, correctly auto-confirmed. **Remaining:** Gap #2 — fail-closed depends on the caller wrapping each ability in the `Promise.race` timeout; the **executor stage** must self-apply that wrapper (not the harness). See `docs/known-issues.md` §5 T1.
- **Source:** `.claude/skills/game-test/SKILL.md` `installGameTestHarness()`; `docs/known-issues.md` §5 T1.

### A5 — vetted state-injection helper · **RESOLVED 2026-07-03** (`harness-hardening` branch)
- **What:** built `window.gtInject` (game-test SKILL.md `installGameTestInjector()`) — a fixed injection API (NOT free-form eval) filling the A2 contract's `setup` slot. `gtInject.set(name, value)` assigns to the **real** binding via a vetted per-name get/set registry (lexical `let`s — `citySize`/`cityReserveAttack`/`totalAttackPoints`/`cumulativeAttackPoints`/`totalRecruitPoints`/`cumulativeRecruitPoints`/`gameMode` — via STRICT set arrows; the `var` `mastermindReserveAttack` via `window[name]`). `gtInject.coldStart(name)` wipes any leftover window property to force the real init path.
- **The traps it closes (both verified live 2026-07-03):** (1) `window.citySize = 7` silently no-ops the top-level `let`; (2) a bare `name = value` for a name with no global binding (e.g. `selectedScheme` — no global exists, only `getSelectedScheme()`) creates an implicit global that **masks** a missing-init bug (the `transformScheme()` incident).
- **Fail-loud, never masks:** `set` throws — creating no implicit global — on an unvetted name, a gone `let`/`var` declaration (strict `ReferenceError`), a lexical write that leaks onto `window`, or a read-back ≠ value. The auto-loop's `setup` MUST route through this; ad-hoc `browser_evaluate` assignment is contract-invalid.
- **Verified live** (whatif, setup screen): `set('citySize',777)` changed the lexical binding a bare `window.citySize=999` left at 5 (no window leak); `set('selectedScheme',…)` threw with **no** masking global (contrast: bare assign made one); `coldStart` cleared a planted mask; `set('mastermindReserveAttack',13)` set the `var`; strict nonexistent-assign throws `ReferenceError` and never masks. Console clean (sw.js-404 pair only).
- **Source:** `.claude/skills/game-test/SKILL.md` (*Vetted state-injection helper (A5)*); `docs/known-issues.md` §5 T1.
...
> **SUPERSEDED 2026-07-03 (same day, later work):** No longer a manual babysit. The executor is now wired as the Phase-4d gate and gates on `gtRunTestCase()`'s fail-closed machine verdict, backed by the A3 console gate, A4 popup auto-drive, and A5 injector — all built and merged to local master. See `docs/automation-initiative.md` §2. The manual-babysit framing below predates that build.
