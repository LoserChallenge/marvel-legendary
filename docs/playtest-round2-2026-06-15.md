...
## Round-3 re-playtest (Paul hands-on, 2026-06-19) — post R2-3-corrected + R2-12

Clean-cache re-playtest after R2-3 CORRECTED (a061c94) + R2-12 (ba62b0f) landed + gated.

- **R2-3 CONFIRMED FIXED (Paul, live):** defeating HYDRA Base fires War Machine "Military-Industrial Complex" +1 Recruit ✅ AND the KO-a-hero step with empty koPile completes cleanly (R2-1 stays fixed) ✅.
- **Carnival of Wonders / War Machine — CONFIRMED NOT A BUG (closed).** Paul fought Carnival of Wonders (Lethal Legion group Location) with War Machine active; no +1 Recruit. This is CORRECT-BY-DESIGN. rules-oracle ruling (SETTLED, cached in docs/rules-notes/revelations.md "Lethal Legion GROUP-Locations" entry): Carnival's "5" is a **Recruit cost**, not Attack — it's a pure Recruit-paid Location, not a Henchman, so "Locations do not count as Villains" applies and War Machine correctly does NOT fire. The HYDRA Base exception is strictly a Henchman-subtype thing; it does NOT extend to group Locations or "any Attack-fought Location." No code change. (Distinct theoretical case still un-observed: defeating a *real Villain* stacked on a generic Location SHOULD fire War Machine — the original R2-3 line-34 "(a)" framing; never reproduced in play, do NOT spin up unless seen.)

### R2-13. Speed "Break the Sound Barrier" — leftover-placement picker softlocks on its 2nd+ use (NEW, P0 softlock)
- Symptom (Paul, live, fresh cache): card works through draw-2 + per-leftover TOP/BOTTOM correctly, but when ≥3 cards must be placed back (so `pickFromCardsSingleRow` is invoked a 2nd time), the picker re-appears and **"PLACE THIS CARD" does nothing** — a card looks previewed but the button stays dead. Game stuck. (Screenshot: picker showing 2 cards, Speedy Delivery previewed, button unresponsive.)
- STRONG HYPOTHESIS (coordinator, from branch code read — worker to VERIFY empirically before patching): `pickFromCardsSingleRow` (cardAbilitiesSidekicks.js:722) re-adds drag-scroll listeners (`row1.addEventListener("mousedown"/"mouseup"/"mouseleave", ...)` at ~826-840 + `setupDragScrolling(row1)`) to the **persistent** `.card-choice-popup-selectionrow1` element on EVERY call, with no cleanup. On the 2nd+ invocation the stacked stale listeners pollute the `isDragging` flag, so the card-`click` handler's `if (isDragging) { ...; return; }` guard (~793) suppresses the selection click → `selected` stays null → `confirmBtn.disabled` never clears → "PLACE THIS CARD" is permanently disabled. First call works because no stale listeners exist yet.
- WHY IT SLIPPED THE GATE: the live multi-leftover loop was never human-tested — watch-doc item 1 was only "FUNCTIONAL VERIFIED" via auto-pick (didn't exercise the live UI), and the cold-read checked logic, not stacked-listener DOM state across loop iterations.
- AFFECTS: any `pickFromCardsSingleRow` caller invoked in a loop — Speed "Break the Sound Barrier" (≥3 leftovers) AND `chooseReturnOrderSingleRow` (Spider-Man-style multi-card reorder) likely share the defect. Fix at the picker, not just Speed.
- FIX DIRECTION (worker decides after verifying): register the drag-scroll/gradient listeners ONCE (or remove-before-add / use a guard flag on row1), or reset `isDragging`/`isDown` state reliably per call so the click guard can't get stuck true. Reproduce via /game-test (force ≥5 revealed so ≥3 go back), confirm the 2nd picker's card-clicks register, then verify both modes. Per-fix gate as usual (cold-read + validator if expansion file touched + smoke).
...
### R2-12. Scarlet Witch "Warp Time and Space" auto-bottoms the returned cards (no top/bottom/order choice) — R2-9 family, but on the HERO deck
- REPRO (Paul, 2026-06-15): gained "Warp Time and Space" (defeated the SW villain → gained as Hero), played it. Card text: reveal top 3 of the HERO deck, keep one in hand, return the other two to the **top or bottom in any order**. The game AUTO-PLACES the two on the bottom with no options.
- CONFIRMED CAUSE: `scarletWitchWarpTimeAndSpace` (expansionRevelations.js:1899) logs "Rest placed on bottom of Hero Deck" (line ~1936) — it hard-bottoms the leftovers, no picker. Same defect family as R2-9 (Speed), which got the `pickFromCardsSingleRow` + `handleCardPlacement` (TOP/BOTTOM, any order) loop. ALSO CHECK `scarletWitchWarpTimeAndSpaceSuper` (line 1946) — likely the same gap.
- IMPORTANT DIFFERENCE FROM R2-9: Warp Time operates on the **HERO deck** (`heroDeck`), not the player deck. The R2-9 helpers (`handleCardPlacement` / `chooseReturnOrderSingleRow`, cardAbilitiesSidekicks.js) are HARDCODED to `playerDeck` (and `handleCardPlacement`'s TOP path carries a player-deck-only `stingOfTheSpider` side effect). So the worker must EITHER parameterize the deck target OR write a small heroDeck-targeted top/bottom-any-order picker — do NOT blindly reuse the playerDeck helpers. Confirm on a clean cache (Paul is now cache-aware). Mode-agnostic.
