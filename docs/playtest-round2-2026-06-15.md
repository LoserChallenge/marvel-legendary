...
### R2-10. House of M extra-draw genuinely broken — REOPENS R2-2/Bug-3 (was wrongly closed as "not a bug")
- REPRO (Paul, 2026-06-15, clean + narrow): GS, House of M, FIRST villain card = Scheme Twist, no Scarlet Witch in city yet → houseOfMTwist hits the `<2 SW` branch and should "play another card from the Villain Deck" (the extra). The extra card does NOT appear. Expected turn-1 sequence: twist + extra + card#2 = 3 cards; actual = 2.
- WHY THE EARLIER CODE-READ MISSED IT: houseOfMTwist (expansionRevelations.js:3496) DOES call `await processVillainCard()` in the `<2 SW` branch, so the code "looks correct." But `processVillainCard` (script.js:5863) wraps processing in a try/catch whose handler is `console.error` ONLY (script.js:5908-5909) — invisible to the player. Hypothesis: processing the extra card throws (likely a re-entrancy interaction with the twist's mid-flight globals — `schemeTwistChainDepth` / `pendingHeroKO` / `schemeTwistTuckComplete`, script.js:5898-5906) and is swallowed there → the card is `pop()`'d off the deck (5869) but never placed → "no extra card," no visible error, deck silently short by one. The R2-2 fix surfaced handleSchemeTwist's catch but NOT processVillainCard's own catch, so this path stayed invisible.
- WORKER: reproduce via Playwright (stack the deck so card 1 = a House of M Scheme Twist with 0 SW in city) and CAPTURE THE F12 CONSOLE — confirm the swallowed throw + identify what throws in the nested draw. Fix the underlying throw; also surface processVillainCard's catch (onscreenConsole, not console.error) so future failures aren't invisible. Mode-agnostic (extra-draw logic has no gameMode branch) — verify both modes. This is the real defect behind the Round-1 "extra card sometimes missing" reports; do NOT re-close without an empirical repro.
