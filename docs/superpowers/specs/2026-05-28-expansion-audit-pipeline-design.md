...
| (c) Mode compliance | Code paths exercised by the card must produce correct behavior under BOTH `gameMode === 'golden'` AND `gameMode === 'whatif'` — neither mode silently breaks the card |
...
- **Output:** per-mechanic prior-art catalog appended as a new section to the mechanics doc. Implementer reads it in `/new-expansion` Phase 1. For each new mechanic: file:line pointers to existing implementations, a 1–2 sentence summary of how they work, recommendation (`REUSE: …` / `ADAPT: …` / `BUILD NEW (no precedent)`)
...
- **Process:** for each engine function listed, identify (1) what state it mutates, (2) all readers of that state, (3) whether each reader pulls from the authoritative source or a stale source (e.g., `transformScheme` writes `window.selectedScheme` but readers query the DOM radio button → gap)
- **Output:** state-propagation gap list + engine-touchpoint misuse list, each finding tagged with an ID (`E-1`, `E-2`, …) that card-type auditors reference
...
**Behavior:**
1. Verify prerequisites — finalized inventory exists in `docs/card-inventory/final/<expansion>.md`, expansion JS file exists, `cardDatabase.js` has the expansion's entries
2. Run `engine-integration-auditor` → capture output as input artifact (`<expansion>-engine-findings.md` in temp)
3. Run `expansion-validator` on the expansion JS → capture output
4. Parallel-dispatch the six card-type auditors with engine-integration findings as context
...
**`/analyze-expansion` — new final step (Step 5):**
- After mechanics reference doc is produced (existing Steps 1–4), run `pattern-reuse-scout`
- Append prior-art catalog as a new section to the mechanics doc
...
9. Inaugural run: execute full pipeline on Revelations
