...
- **Recommendation (coordinator):** LEAVE at 5 (Golden default) — consistent with all other expansions, matches Golden Solo's traditional count, and schemes don't normally dictate hero count. Forcing literal-3 would make SWV1 the lone exception and naively honoring 3 would drop EVERY base/prior `requiredHeroes:3` scheme 5→3 in Golden (wide blast radius); only do it (gated to SWV1) if a specific SWV1 scheme is designed around 3 heroes. **Pending Paul's gameplay call.**
- **Status:** reported blocker RESOLVED; residual design Q open (recommend leave-at-5). Not blocking Phase 4.
...
- **Status:** open — **fail-fast mitigation added (a); elimination still pending (b/c).** The timeout converts hangs/teardowns into fast labelled failures, but does NOT auto-resolve selection popups. VERIFY the mitigation on a real worker game-test run (the Phase-4 testing exercises it heavily) before claiming the teardown symptom is gone — do not re-declare eliminated on assumption (see process note). Not blocking the active build.
