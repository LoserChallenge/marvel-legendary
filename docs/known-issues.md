...
- **Gap #2 — fail-closed depends on the caller wrapping the ability in `Promise.race` (not self-enforced).** The `POPUP-TIMEOUT` safety net only fires if each awaited ability is invoked inside the timeout wrapper; nothing forces it. **The executor stage must apply the ability-timeout wrapper itself — fail-closed cannot depend on caller discipline.** (Assigned to the executor stage, not the harness.)
