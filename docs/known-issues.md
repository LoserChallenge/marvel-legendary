...
- **Status:** open — **fail-fast mitigation (a) VERIFIED working on one real instance; elimination still pending (b/c).** The timeout converts hangs/teardowns into fast labelled failures, but does NOT auto-resolve selection popups. **Verification (Phase-4 BATCH 4, 2026-06-27):** a real "THE HOOD — MASTER STRIKE" selection popup hung on `onBeginGame`; the in-page `Promise.race` timeout fired, the worker caught a labelled rejection, and **the browser env survived** (subsequent evaluates ran on the same page). That confirms the mitigation's claimed behavior (hang → fast labelled failure, env stays alive) on a genuine hang — not just "less frequent." Hardening (b) selection-driving helper + (c) re-install-after-navigation remain open (a cc-helper side-session is scoping these). Not blocking the active build.
