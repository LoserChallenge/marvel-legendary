...
- **Status:** ✅ **RESOLVED 2026-07-03** (`harness-hardening` branch) — all three parts landed and live-verified. (a) hard-timeout fail-fast (Phase-4 BATCH 4, 2026-06-27: a real "THE HOOD — MASTER STRIKE" selection popup hung on `onBeginGame`; the in-page `Promise.race` fired, worker caught a labelled rejection, env survived). (b) selection-driving + (c) re-install-after-navigation (`installGameTestHarness()`; verified against the real `KO1To4FromDiscard` picker — named-pick-not-default, re-entrant 2nd-use, empty-queue→POPUP-TIMEOUT env-alive, post-reload re-install). Fail-closed preserved throughout: undeclared/unmatched selection is never guessed. **Residual follow-up (NOT A4, tracked in game-test SKILL.md scope note):** sibling selection popups (`order-choice`/`draw-choice`/`card-choice-city-hq`) still blind-auto-confirm.
