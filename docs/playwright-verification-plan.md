...
- ⏳ **Piece 2 (remaining) 1A work — city resize (E-4):** city-size resize for Earthquake (→ 7 at setup) and Tsunami (→ 3 on transform). Scoping recommends `destroyedSpaces[]`-flag model over physical array realloc; build `resizeCityForScheme()`, call from both twist handlers, wire setup so Earthquake starts at 7. Separate dispatch — see Fix 1A scope report.
